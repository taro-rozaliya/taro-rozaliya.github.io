...
mozCatalogFilter.prototype.doSortOrFilter = function(origin)
{
    ...
    var url = new URL(window.location),
        ...
        newFilterValues = this.getCurrentFilterValues();
    ...
    if (origin == 'filter') {
        ...
        newFilterValues.forEach(function(value) {
            if (Array.isArray(value.value)) {
                value.value.forEach(function(v) {
                    url.searchParams.append(value.key + '[]', v);
                });
            }
            else {
                url.searchParams.set(value.key, value.value);
            }
        });
    }
